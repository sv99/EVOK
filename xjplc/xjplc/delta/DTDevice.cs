...
namespace xjplc
{
    ...
    public class DTDevice 
    {
        ...
        private List<DTPlcInfo> InsertPlcInfo(List<DTPlcInfo> plcInfoLst0)
        {
            List<DTPlcInfo> plcInfoLst1 = new List<DTPlcInfo>();
            ...
            for (int i = 0; i < plcInfoLst0.Count - 1; i++)
            {
                ...
                if (((plcInfoLst0[i + 1].RelAddr - plcInfoLst0[i].RelAddr) < 2) && (plcInfoLst0[i + 1].IntArea == plcInfoLst0[i].IntArea))
                {
                    int m = 0;
                    m = plcInfoLst0[i + 1].RelAddr - plcInfoLst0[i].RelAddr;
                    if (m > 1)
                    {
                        DTPlcInfo[] tmpPlcInfoLst = DTPLCcmd.GetPlcInfo(plcInfoLst0[i].RelAddr, m, plcInfoLst0[i].StrArea, plcInfoLst0[i].ValueMode);
            ...
            plcInfoLst1 = plcInfoLst1.Union(plcInfoLst0.ToArray()).ToList<DTPlcInfo>();

            //排序
            plcInfoLst1= plcInfoLst1.OrderBy(x => x.AbsAddr).ToList();
            //去重复
            plcInfoLst1 = plcInfoLst1.Distinct(new ModelComparerDT()).ToList();
        ...
        private void UpdateUI0()
        {
            ...
            foreach (DataRow row in DataForm.Rows)
            {
                ...
                mArea = DTPLCPackCmdAndDataUnpack.AreaGetFromStr(area);
                int[] pos = new int[2];
