...
namespace xjplc
{
    public class EvokWork
    {
        ...
        //定义后要加入集合 
        public PlcInfoSimple prodOutInPs = new PlcInfoSimple("总产量读写"); //忽略寄存器的影响直接匹配参数名
        ...
        public PlcInfoSimple pageShiftPs = new PlcInfoSimple("页面切换写");
       ...
       PsLstHand = new List<PlcInfoSimple>();
        ...
        public void pause()
        {
            evokDevice.SetMValueOFF2ON(stopOutPs);
        }
        //自动上料
        public void autoSL()
        {
        ...
        private void CutWork0()
        {
            ...
            {             
                for (int i = CutProCnt; i < optSize.ProdInfoLst.Count; i++)
                {
                    ...
                    //打条码
                    // evokWork.printBarcode(report1,optSize.SingleSizeLst[i][0].ParamStrLst.ToArray());
                    int oldcntcut = 0;//保存的老计数值
                    ...
                    while (RunFlag)
                    {
                        ...
                        Thread.Sleep(10);
                        int c = cutDoneOutInPs.ShowValue;
                        ...
                        if (c != oldcntcut && oldcntcut < optSize.ProdInfoLst[i].Cut.Count)
                        {
                            int no = 0;

                            if (int.TryParse(optSize.SingleSizeLst[i][oldcntcut].DtUser.Rows[optSize.SingleSizeLst[i][oldcntcut].Xuhao]["已切数量"].ToString(), out no))
                            {
                                no++;
                                optSize.SingleSizeLst[i][oldcntcut].DtUser.Rows[optSize.SingleSizeLst[i][oldcntcut].Xuhao]["已切数量"] = no;
                            }

                            ConstantMethod.ShowInfo(rtbWork, "第" + (oldcntcut + 1).ToString() + "段尺寸：" + optSize.ProdInfoLst[i].Cut[oldcntcut].ToString() + "-----完成");

                            oldcntcut = c;
                            //打条码
                            // evokWork.printBarcode(report1,optSize.SingleSizeLst[i][nowCnt].ParamStrLst.ToArray());                          
                        }

                        if (c >= optSize.ProdInfoLst[i].Cut.Count) break;
        ...
        public bool ShiftPage(int pageid)
        {
            if (evokDevice.Status == Constant.DeviceConnected)
            {
                ...
                if (pageid == 0)
                {
                    evokDevice.SetDValue(pageShiftPs,2);
                }
                if (pageid == 1)
                {
                    evokDevice.SetDValue(pageShiftPs, 3);
                }
